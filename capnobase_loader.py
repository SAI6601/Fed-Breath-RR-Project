...
The ground-truth respiratory rate (RR) label is the mean of the CO₂-derived
...
def _print_nested(name, obj, indent=0):
    ...
    if hasattr(obj, '_fieldnames'):
        # MATLAB struct
        print(f"{prefix}📦 {name}  (struct, fields: {obj._fieldnames})")
...
def _find_rr_label(mat: dict):
    ...
    CapnoBase provides CO₂-derived instantaneous RR under:
...
class CapnoBaseDataset(Dataset):
    ...
    def __init__(self, data_dir: str = CAPNO_DIR):
        ...
        if not os.path.isdir(data_dir):
            print(f"⚠️  CapnoBase directory not found: {data_dir}")
        ...
        print(f"✅ Found {len(self.files)} CapnoBase .mat files in {data_dir}")
...
if __name__ == "__main__":
    ds = CapnoBaseDataset()
    if len(ds) == 0:
        print("\n⚠️  No .mat files found. Place CapnoBase files in data/capnobase/")
    else:
